...
import CreateGuestAddressModal from "./CreateGuestAddressModal";
...
const GuestAddress: React.FC<AddressProps> = ({ setAddress }) => {
  //   const [savedAddress, setSavedAddress] = useState<any[]>([]);
  const { guestSavedAddress, setGuestSavedAddress } = useShop();
  ...
  useEffect(() => {
    if (guestSavedAddress.length === 1 && !selectedAddress) {
      const onlyAddress = guestSavedAddress[0];
      setSelectedAddress(onlyAddress._id);
      setAddress(onlyAddress);
    }
  }, [guestSavedAddress, selectedAddress, setAddress]);

  useEffect(() => {
    if (guestSavedAddress.length === 1) {
  ...
  }, [guestSavedAddress.length]);

  const displayedItems = showAll
    ? guestSavedAddress
    : guestSavedAddress.slice(0, 2);
  ...
  return (
    <div className="w-full">
          ...
          <>
            ...
            {guestSavedAddress.length > 2 && (
              ...
              >
                {showAll
                  ? "Show Less"
                  : `Show ${guestSavedAddress.length - 2} More`}
              </button>
            )}
          </>
        ) : (
        ...
        >
          ...
          <span className="text-sm uppercase tracking-widest text-gray-600 group-hover:text-gray-900">
            <p className="text-xs text-gray-500">
              {guestSavedAddress.length === 1 &&
                (toggle
                  ? "Click to create a new address."
                  : "address selected automatically.")}
              {guestSavedAddress.length === 0 && "create a new address."}
              {guestSavedAddress.length > 1 && "create a new address."}
            </p>
          </span>
        </button>
      </div>
      ...
      {showModal && (
        <CreateGuestAddressModal
          setShowModal={setShowModal}
          guestSavedAddress={guestSavedAddress}
          setGuestSavedAddress={setGuestSavedAddress}
