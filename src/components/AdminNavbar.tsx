...
import { mainLogo, smallLogo } from "../assets/index";
          ...
          >
            {navLink.subMenuLinks?.map((subMenuLink, index) => (
              <Link
                key={index}
                to={subMenuLink.url}
                className="block py-1 px-3 text-center text-gray-700 hover:bg-gray-100 dark:text-gray-300 dark:hover:bg-gray-700 lg:text-left rounded-md transition-colors duration-200 z-[1000px]"
                onClick={() => setIsMobileOpen(false)}
              >
                {subMenuLink.title}
              </Link>
